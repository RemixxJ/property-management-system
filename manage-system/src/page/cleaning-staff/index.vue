  ...
  <section class="super-administrator">
    <el-table :data="tableData" style="width: 100%" height="500" border>
      ...
      <el-table-column label="清洁状态" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.qjStatus == '1' ? '待清洁' : '暂无清洁' }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="维修上报" :visible.sync="dialogFormVisible">
      <el-form :model="form" ref="addRef" :rules="rules">
        ...
        <el-form-item label="清洁图片上传" :label-width="formLabelWidth" prop="qjimg">
          <el-upload
            action="/upload"
            :auto-upload="false"
...
<script>
import { getCleaningHouse, adminHouseEdit } from '@/api/superAdminLogin.js'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        qjimg: [{ required: false, message: '请上传', trigger: 'change' }]
      },
      optionsqj: [
        {
          value: '0',
  ...
  methods: {
    ...
    adminHouseEdit(formName) {
      this.$refs[formName].validate(async (valid) => {
        if (valid) {
          this.form = { ...this.form, qjimg: this.form.qjimg.toString() }
          const params = {
            ...this.curRow,
            ...this.form
          }
    ...
    async getCleaningHouse() {
      const res = await getCleaningHouse()
      console.log(res)
    ...
    fileSuccess(res, file) {
      this.form.qjimg = this.form.qjimg.push(res.data.data.fileId)
    },
    uploadImg(file) {
      console.log(file)
      ...
      axios({
        ...
        this.form.qjimg.push(res.data.fileId)
        console.log(res)
      })
    }
  }
