...
let findUsername = (username) => {
  let sql = `select * from maintenancePersonnel where username=?`
  let sqlArr = [username]
  return connection.SySqlConnect(sql, sqlArr)
}
//用户注册
maintenancePersonnelRegister = (req, res) => {
  ...
  findUsername(username).then((r) => {
    if (r.length > 0) {
      ...
      if (!username) {
        ...
        let sql = `insert into maintenancePersonnel(username,password,nickname,status,create_time) value(?,?,?,?,?)`
...
//login
maintenancePersonnelLogin = (req, res) => {
  let { username, password } = req.body
  let sql = `select * from maintenancePersonnel where username=? and password=?`
...
//获取房屋待维修房屋
getMaintainHouse = (req, res) => {
  //wxStatus  0 无维修  1  待维修
  let wxStatus = '1'
  let sql = `select * from house where wxStatus=?`
  let sqlArr = [wxStatus]
  connection
...
module.exports = { maintenancePersonnelRegister, maintenancePersonnelLogin, getMaintainHouse }
